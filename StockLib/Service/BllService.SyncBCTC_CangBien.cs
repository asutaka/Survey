    ...
    public partial class BllService
    {
        public async Task SyncBCTC_CangBien()
        {
            try
            {
                ...
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.CangBien)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_CangBien_KQKD(item);
                    await SyncBCTC_CangBien_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_CangBien|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_CangBien_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_CangBien> filter = null;
                        var builder = Builders<Financial_CangBien>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_CangBien>>
                            {
                        ...
                        var lUpdate = _cangbienRepo.GetByFilter(filter);
                        Financial_CangBien entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_CangBien
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _cangbienRepo.InsertOne(entityUpdate);
                        }
                        ...
                        _cangbienRepo.Update(entityUpdate);
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_CangBien_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_CangBien_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_CangBien_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
