    ...
    public partial class BllService
    {
        public async Task SyncBCTC_Go()
        {
            try
            {
                ...
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.Go)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_Go_KQKD(item);
                    await SyncBCTC_Go_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_Go|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Go_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_Go> filter = null;
                        var builder = Builders<Financial_Go>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_Go>>
                            {
                        ...
                        var lUpdate = _goRepo.GetByFilter(filter);
                        Financial_Go entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_Go
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _goRepo.InsertOne(entityUpdate);
                        }
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Go_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Go_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Go_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
