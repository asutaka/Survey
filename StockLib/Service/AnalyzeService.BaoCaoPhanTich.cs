    ...
    public partial class AnalyzeService
    {
        public async Task<(int, string)> BaoCaoPhanTich(DateTime dt)
        {
            try
            {
                ...
                var lSSI = await _apiService.SSI_GetPost(false);
                if (lSSI != null)
                {
                ...
                {
                    var t = int.Parse($"{time.Year}{time.Month.To2Digit()}{time.Day.To2Digit()}");
                    var lValid = lVNDirectIndustry.Where(x => int.Parse(x.newsDate.Replace("-", "")) >= t);
                    if (lValid?.Any() ?? false)
                    {
                        foreach (var itemValid in lValid)
                        {
                ...
                {
                    var lValid = lAgribankIndustry.Where(x => x.Date > time);
                    if (lValid?.Any() ?? false)
                    {
                        foreach (var itemValid in lValid)
                        {
                ...
                var lSSIIndustry = await _apiService.SSI_GetPost(true);
                if (lSSIIndustry != null)
                {
                    var lValid = lSSIIndustry.Where(x => x.date >= time);
                    if (lValid?.Any() ?? false)
                    {
                        foreach (var itemValid in lValid)
                        {
                            ...
                            sBuilder.AppendLine($"[SSI - Báo cáo ngành] {itemValid.title}");
                            sBuilder.AppendLine($"Link: https://www.ssi.com.vn/khach-hang-ca-nhan/bao-cao-nganh");
                        }
                    }
                }

                var lFPTSIndustry = await _apiService.FPTS_GetPost(true);
                if (lFPTSIndustry != null)
                {
                    var lValid = lFPTSIndustry.Where(x => x.date >= time);
                    if (lValid?.Any() ?? false)
                    {
                        foreach (var itemValid in lValid)
                        {
                ...
                var lBSCIndustry = await _apiService.BSC_GetPost(true);
                if (lBSCIndustry != null)
                {
                    var lValid = lBSCIndustry.Where(x => x.date >= time);
                    if (lValid?.Any() ?? false)
                    {
                        foreach (var itemValid in lValid)
                        {
                            ...
                            sBuilder.AppendLine($"[BSC - Báo cáo ngành] {itemValid.title}");
                            if (string.IsNullOrWhiteSpace(itemValid.path))
                            {
                                sBuilder.AppendLine($"Link: https://www.bsc.com.vn/bao-cao-phan-tich/danh-muc-bao-cao/2");
                            }
                            else
                            {
                                sBuilder.AppendLine($"Link: {itemValid.path}");
                ...
                var lMBSIndustry = await _apiService.MBS_GetPost(true);
                if (lMBSIndustry != null)
                {
                    var lValid = lMBSIndustry.Where(x => x.date >= time);
                    if (lValid?.Any() ?? false)
                    {
                        foreach (var itemValid in lValid)
                        {
                ...
                var lPSIIndustry = await _apiService.PSI_GetPost(true);
                if (lPSIIndustry != null)
                {
                    var lValid = lPSIIndustry.Where(x => x.date >= time);
                    if (lValid?.Any() ?? false)
                    {
                        foreach (var itemValid in lValid)
                        {
                            ...
                            if (string.IsNullOrWhiteSpace(itemValid.path))
                            {
                                sBuilder.AppendLine($"Link: https://www.psi.vn/vi/trung-tam-phan-tich/bao-cao-nganh");
                            }
                            else
                            {
                                sBuilder.AppendLine($"Link: {itemValid.path}");
