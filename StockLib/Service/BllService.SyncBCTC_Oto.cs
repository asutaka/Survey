    ...
    public partial class BllService
    {
        public async Task SyncBCTC_Oto()
        {
            try
            {
                ...
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.Oto || x.ty == (int)EStockType.OtoTai)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_Oto_KQKD(item);
                    await SyncBCTC_Oto_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_Oto|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Oto_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_Oto> filter = null;
                        var builder = Builders<Financial_Oto>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_Oto>>
                            {
                        ...
                        var lUpdate = _otoRepo.GetByFilter(filter);
                        Financial_Oto entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_Oto
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _otoRepo.InsertOne(entityUpdate);
                        }
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Oto_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Oto_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Oto_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
