namespace StockLib.Service.Settings
{
    public static class ServiceSetting
    {
        public const string _botToken = "7422438658:AAEPzAwq-5rA-5dLEFRpPrdOBt9yMfnkBxA";
        public const string _botTokenCoin = "7783423206:AAGnpRM_8xnxr44sbgOX-ktIHXQEsyMxH6A";
        public const string _chart = "https://export.highcharts.com";//Lấy chart dạng ảnh
        public const string _chartLocal = "http://127.0.0.1:7801";//Lấy chart dạng ảnh(tự host với NodeJS)
    }
}
