...
namespace StockLib.Service
{
    ...
    {
        public async Task<List<Stream>> Chart_Nhua(IEnumerable<string> lInput)
        {
            try
            {
                ...
                var lFinancial = _nhuaRepo.GetByFilter(Builders<Financial_Nhua>.Filter.Eq(x => x.d, time.Item1));
                if (!lFinancial.Any())
                    return null;

                var streamTonkho = await Chart_Nhua_TonKho(lInput, lFinancial);
            ...
            {
                _logger.LogError($"BllService.Chart_Nhua|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_Nhua_TonKho(IEnumerable<string> lInput, List<Financial_Nhua> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
                var lOrderBy = new List<Financial_Nhua>();
            ...
            {
                _logger.LogError($"BllService.Chart_Nhua_TonKho|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_Nhua_NoTaiChinh(IEnumerable<string> lInput, List<Financial_Nhua> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
        ...
        {
            var lFinancial = _nhuaRepo.GetByFilter(Builders<Financial_Nhua>.Filter.Eq(x => x.s, code));
            if (!lFinancial.Any())
                return null;
            ...
            var streamTonKho = await Chart_Nhua_TonKho(lFinancial, code);
            var streamNoTaiChinh = await Chart_Nhua_NoTaiChinh(lFinancial, code);
        ...
        private async Task<Stream> Chart_Nhua_TonKho(List<Financial_Nhua> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
        ...
        private async Task<Stream> Chart_Nhua_NoTaiChinh(List<Financial_Nhua> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_Nhua_NoTaiChinh|EXCEPTION| {ex.Message}");
            }
            return null;
        }
