        ...
        public async Task SyncBCTC_Logistic()
        {
            try
            {
                ...
                foreach (var item in lStockFilter)
                {
                    ...
                    await SyncBCTC_Logistic_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_Logistic|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Logistic_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_Logistic> filter = null;
                        var builder = Builders<Financial_Logistic>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_Logistic>>
                            {
                        ...
                        var lUpdate = _logisticRepo.GetByFilter(filter);
                        Financial_Logistic entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_Logistic
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _logisticRepo.InsertOne(entityUpdate);
                        }
                        ...
                        _logisticRepo.Update(entityUpdate);
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Logistic_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Logistic_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Logistic_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
