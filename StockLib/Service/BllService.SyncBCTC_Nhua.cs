...
namespace StockLib.Service
{
    public partial class BllService
    {
        public async Task SyncBCTC_Nhua()
        {
            try
            {
                var lStockFilter = StaticVal._lNhua;
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_Nhua_KQKD(item);
                    await SyncBCTC_Nhua_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_Nhua|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Nhua_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_Nhua> filter = null;
                        var builder = Builders<Financial_Nhua>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_Nhua>>
                            {
                        ...
                        var lUpdate = _nhuaRepo.GetByFilter(filter);
                        Financial_Nhua entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_Nhua
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _nhuaRepo.InsertOne(entityUpdate);
                        }
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Nhua_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Nhua_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Nhua_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
