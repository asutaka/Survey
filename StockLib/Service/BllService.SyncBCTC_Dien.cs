...
namespace StockLib.Service
{
    ...
    {
        public async Task SyncBCTC_Dien(bool onlyLast = false)
        {
            try
            {
                var lStockFilter = StaticVal._lStock.Where(x => x.status == 1 && (x.cat.Any(x => x.ty == (int)EStockType.DienGio 
                ...
                {
                    await SyncBCTC_Dien_KQKD(item, onlyLast);
                    await SyncBCTC_Dien_CDKT(item, onlyLast);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_Dien|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Dien_KQKD(string code, bool onlyLast)
        {
            try
            {
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Dien_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Dien_CDKT(string code, bool onlyLast)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Dien_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
