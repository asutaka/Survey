...
namespace StockLib.Service
{
    ...
    {
        public async Task<List<Stream>> Chart_DauKhi(IEnumerable<string> lInput)
        {
            try
            {
                ...
                var lFinancial = _daukhiRepo.GetByFilter(Builders<Financial_DauKhi>.Filter.Eq(x => x.d, time.Item1));
                if (!lFinancial.Any())
                    return null;

                var streamTonkho = await Chart_DauKhi_TonKho(lInput, lFinancial);
                lOutput.Add(streamTonkho);

                var streamNo = await Chart_DauKhi_NoTaiChinh(lInput, lFinancial);
            ...
            {
                _logger.LogError($"BllService.Chart_DauKhi|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_DauKhi_TonKho(IEnumerable<string> lInput, List<Financial_DauKhi> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
                var lOrderBy = new List<Financial_DauKhi>();
            ...
            {
                _logger.LogError($"BllService.Chart_DauKhi_TonKho|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_DauKhi_NoTaiChinh(IEnumerable<string> lInput, List<Financial_DauKhi> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_DauKhi_No|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<List<Stream>> Chart_DauKhi(string code)
        {
            var lFinancial = _daukhiRepo.GetByFilter(Builders<Financial_DauKhi>.Filter.Eq(x => x.s, code));
            if (!lFinancial.Any())
                return null;
            ...
            var streamTonKho = await Chart_DauKhi_TonKho(lFinancial, code);
            var streamNoTaiChinh = await Chart_DauKhi_NoTaiChinh(lFinancial, code);
        ...
        private async Task<Stream> Chart_DauKhi_TonKho(List<Financial_DauKhi> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
        ...
        private async Task<Stream> Chart_DauKhi_NoTaiChinh(List<Financial_DauKhi> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_DauKhi_NoTaiChinh|EXCEPTION| {ex.Message}");
            }
            return null;
        }
    }
