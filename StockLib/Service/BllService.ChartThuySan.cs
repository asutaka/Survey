    ...
    public partial class BllService
    {
        public async Task<List<Stream>> Chart_ThuySan(IEnumerable<string> lInput)
        {
            try
            {
                ...
                var lFinancial = _thuysanRepo.GetByFilter(Builders<Financial_ThuySan>.Filter.Eq(x => x.d, time.Item1));
                if (!lFinancial.Any())
                    return null;
        ...
        private async Task<Stream> Chart_ThuySan_TonKho(IEnumerable<string> lInput, List<Financial_ThuySan> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
                var lOrderBy = new List<Financial_ThuySan>();
        ...
        private async Task<Stream> Chart_ThuySan_NoTaiChinh(IEnumerable<string> lInput, List<Financial_ThuySan> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
        ...
        {
            var lFinancial = _thuysanRepo.GetByFilter(Builders<Financial_ThuySan>.Filter.Eq(x => x.s, code));
            if (!lFinancial.Any())
                return null;
            ...
            var streamTonKho = await Chart_ThuySan_TonKho(lFinancial, code);
            var streamNoTaiChinh = await Chart_ThuySan_NoTaiChinh(lFinancial, code);
        ...
        private async Task<Stream> Chart_ThuySan_TonKho(List<Financial_ThuySan> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
        ...
        private async Task<Stream> Chart_ThuySan_NoTaiChinh(List<Financial_ThuySan> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_ThuySan_NoTaiChinh|EXCEPTION| {ex.Message}");
            }
            return null;
        }
