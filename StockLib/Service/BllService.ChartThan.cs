...
namespace StockLib.Service
{
    ...
    {
        public async Task<List<Stream>> Chart_Than(IEnumerable<string> lInput)
        {
            try
            {
                ...
                var lFinancial = _thanRepo.GetByFilter(Builders<Financial_Than>.Filter.Eq(x => x.d, time.Item1));
                if (!lFinancial.Any())
                    return null;

                var streamTonkho = await Chart_Than_TonKho(lInput, lFinancial);
                lOutput.Add(streamTonkho);

                var streamNo = await Chart_Than_NoTaiChinh(lInput, lFinancial);
                lOutput.Add(streamNo);

                var streamXK = await Chart_XuatKhau_Than();
            ...
            {
                _logger.LogError($"BllService.Chart_Than|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_Than_TonKho(IEnumerable<string> lInput, List<Financial_Than> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
                var lOrderBy = new List<Financial_Than>();
            ...
            {
                _logger.LogError($"BllService.Chart_Than_TonKho|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_Than_NoTaiChinh(IEnumerable<string> lInput, List<Financial_Than> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_Than_No|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<List<Stream>> Chart_Than(string code)
        {
            var lFinancial = _thanRepo.GetByFilter(Builders<Financial_Than>.Filter.Eq(x => x.s, code));
            if (!lFinancial.Any())
                return null;
            ...
            var streamTonKho = await Chart_Than_TonKho(lFinancial, code);
            var streamNoTaiChinh = await Chart_Than_NoTaiChinh(lFinancial, code);
        ...
        private async Task<Stream> Chart_Than_TonKho(List<Financial_Than> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
        ...
        private async Task<Stream> Chart_Than_NoTaiChinh(List<Financial_Than> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_Than_NoTaiChinh|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_XuatKhau_Than()
        {
            try
            {
                var lThan = _haiquanRepo.GetByFilter(Builders<ThongKeHaiQuan>.Filter.Eq(x => x.key, (int)EHaiQuan.Than)).OrderBy(x => x.d);
                var lSeries = new List<HighChartSeries_BasicColumn>
                {
                    new HighChartSeries_BasicColumn
                    {
                        data = lThan.TakeLast(25).Select(x => x.va),
                        name = "Giá trị xuất khẩu Than",
                        type = "column",
                        dataLabels = new HighChartDataLabel{ enabled = true, format = "{point.y:.1f}" },
                        color = "#012060"
                    }
                ...
                if (lThan.Sum(x => x.price) > 0)
                {
                    lSeries.Add(new HighChartSeries_BasicColumn
                    {
                        data = lThan.TakeLast(25).Select(x => x.price),
                        name = "Giá xuất khẩu",
                        type = "spline",
                        dataLabels = new HighChartDataLabel { enabled = true, format = "{point.y:.1f}" },
                        color = "#C00000",
                ...
                return await Chart_BasicBase($"Xuất khẩu - Thống kê nửa tháng", lThan.TakeLast(25).Select(x => x.d.GetNameHaiQuan()).ToList(), lSeries, "giá trị: triệu USD", "giá trị: USD");
            }
            catch (Exception ex)
            {
