    ...
    public partial class BllService
    {
        public async Task SyncBCTC_Than()
        {
            try
            {
                ...
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.Than)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_Than_KQKD(item);
                    await SyncBCTC_Than_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_Than|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Than_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_Than> filter = null;
                        var builder = Builders<Financial_Than>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_Than>>
                            {
                        ...
                        var lUpdate = _thanRepo.GetByFilter(filter);
                        Financial_Than entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_Than
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _thanRepo.InsertOne(entityUpdate);
                        }
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Than_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_Than_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_Than_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
