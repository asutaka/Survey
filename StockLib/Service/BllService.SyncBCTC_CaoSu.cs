    ...
    public partial class BllService
    {
        public async Task SyncBCTC_CaoSu()
        {
            try
            {
                ...
                {
                    await SyncBCTC_CaoSu_KQKD(item);
                    await SyncBCTC_CaoSu_CDKT(item);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task SyncBCTC_CaoSu_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_CaoSu> filter = null;
                        var builder = Builders<Financial_CaoSu>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_CaoSu>>
                            {
                        ...
                        var lUpdate = _caosuRepo.GetByFilter(filter);
                        Financial_CaoSu entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_CaoSu
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _caosuRepo.InsertOne(entityUpdate);
                        }
                        ...
                        _caosuRepo.Update(entityUpdate);
        ...
        private async Task SyncBCTC_CaoSu_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
