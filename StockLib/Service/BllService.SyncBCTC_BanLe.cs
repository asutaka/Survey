...
namespace StockLib.Service
{
    ...
    {
        public async Task SyncBCTC_BanLe(bool onlyLast = false)
        {
            try
            {
                var lStock = _stockRepo.GetAll();
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.BanLe)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_BanLe_KQKD(item, onlyLast);
                    await SyncBCTC_BanLe_CDKT(item, onlyLast);
                }
            }
            catch (Exception ex)
            {
        ...
        private async Task SyncBCTC_BanLe_KQKD(string code, bool onlyLast)
        {
            try
            {
        ...
        private async Task SyncBCTC_BanLe_CDKT(string code, bool onlyLast)
        {
            try
            {
                var time = GetCurrentTime();
