...
namespace StockLib.Service
{
    public partial class BllService
    {
        public async Task SyncBCTC_PhanBon()
        {
            try
            {
                var lStockFilter = StaticVal._lPhanBon;
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_PhanBon_KQKD(item);
                    await SyncBCTC_PhanBon_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_PhanBon|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_PhanBon_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_PhanBon> filter = null;
                        var builder = Builders<Financial_PhanBon>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_PhanBon>>
                            {
                        ...
                        var lUpdate = _phanbonRepo.GetByFilter(filter);
                        Financial_PhanBon entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_PhanBon
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _phanbonRepo.InsertOne(entityUpdate);
                        }
                        ...
                        _phanbonRepo.Update(entityUpdate);
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_PhanBon_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_PhanBon_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_PhanBon_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
