...
namespace StockLib.Service
{
    ...
    {
        public async Task<List<Stream>> Chart_Go(IEnumerable<string> lInput)
        {
            try
            {
                ...
                var lFinancial = _goRepo.GetByFilter(Builders<Financial_Go>.Filter.Eq(x => x.d, time.Item1));
                if (!lFinancial.Any())
                    return null;

                var streamTonkho = await Chart_Go_TonKho(lInput, lFinancial);
                lOutput.Add(streamTonkho);

                var streamNo = await Chart_Go_NoTaiChinh(lInput, lFinancial);
                lOutput.Add(streamNo);

                var streamXK = await Chart_XuatKhau_Go();
            ...
            {
                _logger.LogError($"BllService.Chart_Go|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_Go_TonKho(IEnumerable<string> lInput, List<Financial_Go> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
                var lOrderBy = new List<Financial_Go>();
            ...
            {
                _logger.LogError($"BllService.Chart_Go_TonKho|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_Go_NoTaiChinh(IEnumerable<string> lInput, List<Financial_Go> lFinancial)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_Go_No|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<List<Stream>> Chart_Go(string code)
        {
            var lFinancial = _goRepo.GetByFilter(Builders<Financial_Go>.Filter.Eq(x => x.s, code));
            if (!lFinancial.Any())
                return null;
            ...
            var streamTonKho = await Chart_Go_TonKho(lFinancial, code);
            var streamNoTaiChinh = await Chart_Go_NoTaiChinh(lFinancial, code);
        ...
        private async Task<Stream> Chart_Go_TonKho(List<Financial_Go> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
        ...
        private async Task<Stream> Chart_Go_NoTaiChinh(List<Financial_Go> lFinancial, string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.Chart_Go_NoTaiChinh|EXCEPTION| {ex.Message}");
            }
            return null;
        }

        private async Task<Stream> Chart_XuatKhau_Go()
        {
            try
            {
                var lGo = _haiquanRepo.GetByFilter(Builders<ThongKeHaiQuan>.Filter.Eq(x => x.key, (int)EHaiQuan.Go)).OrderBy(x => x.d);
                var lSeries = new List<HighChartSeries_BasicColumn>
                {
                    new HighChartSeries_BasicColumn
                    {
                        data = lGo.TakeLast(25).Select(x => x.va),
                        name = "Giá trị xuất khẩu Gỗ",
                        type = "column",
                        dataLabels = new HighChartDataLabel{ enabled = true, format = "{point.y:.1f}" },
                        color = "#012060"
                    }
                ...
                if (lGo.Sum(x => x.price) > 0)
                {
                    lSeries.Add(new HighChartSeries_BasicColumn
                    {
                        data = lGo.TakeLast(25).Select(x => x.price),
                        name = "Giá xuất khẩu",
                        type = "spline",
                        dataLabels = new HighChartDataLabel { enabled = true, format = "{point.y:.1f}" },
                        color = "#C00000",
                ...
                return await Chart_BasicBase($"Xuất khẩu - Thống kê nửa tháng", lGo.TakeLast(25).Select(x => x.d.GetNameHaiQuan()).ToList(), lSeries, "giá trị: triệu USD", "giá trị: USD");
            }
            catch (Exception ex)
            {
