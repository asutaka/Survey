    ...
    public partial class BllService
    {
        public async Task SyncBCTC_DetMay()
        {
            try
            {
                ...
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.DetMay)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_DetMay_KQKD(item);
                    await SyncBCTC_DetMay_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_DetMay|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_DetMay_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_DetMay> filter = null;
                        var builder = Builders<Financial_DetMay>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_DetMay>>
                            {
                        ...
                        var lUpdate = _detmayRepo.GetByFilter(filter);
                        Financial_DetMay entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_DetMay
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _detmayRepo.InsertOne(entityUpdate);
                        }
                        ...
                        _detmayRepo.Update(entityUpdate);
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_DetMay_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_DetMay_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_DetMay_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
