...
namespace StockLib.Service
{
    ...
    {
        public async Task SyncBCTC_ThuySan(bool onlyLast = false)
        {
            try
            {
                var lStock = _stockRepo.GetAll();
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.ThuySan)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_ThuySan_KQKD(item, onlyLast);
                    await SyncBCTC_ThuySan_CDKT(item, onlyLast);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_ThuySan|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_ThuySan_KQKD(string code, bool onlyLast)
        {
            try
            {
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_ThuySan_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_ThuySan_CDKT(string code, bool onlyLast)
        {
            try
            {
                var time = GetCurrentTime();
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_ThuySan_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
