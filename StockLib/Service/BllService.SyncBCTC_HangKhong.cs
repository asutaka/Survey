    ...
    public partial class BllService
    {
        public async Task SyncBCTC_HangKhong()
        {
            try
            {
                ...
                var lStockFilter = lStock.Where(x => x.status == 1 && x.cat.Any(x => x.ty == (int)EStockType.HangKhong)).Select(x => x.s);
                foreach (var item in lStockFilter)
                {
                    await SyncBCTC_HangKhong_KQKD(item);
                    await SyncBCTC_HangKhong_CDKT(item);
                }
            }
            catch (Exception ex)
            {
                _logger.LogError($"BllService.SyncBCTC_HangKhong|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_HangKhong_KQKD(string code)
        {
            try
            {
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        FilterDefinition<Financial_HangKhong> filter = null;
                        var builder = Builders<Financial_HangKhong>.Filter;
                        var lFilter = new List<FilterDefinition<Financial_HangKhong>>
                            {
                        ...
                        var lUpdate = _hangkhongRepo.GetByFilter(filter);
                        Financial_HangKhong entityUpdate = lUpdate.FirstOrDefault();
                        if (lUpdate is null || !lUpdate.Any())
                        {
                            //insert
                            entityUpdate = new Financial_HangKhong
                            {
                                d = int.Parse($"{year}{quarter}"),
                                s = code,
                                t = (int)DateTimeOffset.Now.ToUnixTimeSeconds()
                            };
                            _hangkhongRepo.InsertOne(entityUpdate);
                        }
                        ...
                        _hangkhongRepo.Update(entityUpdate);
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_HangKhong_KQKD|EXCEPTION| {ex.Message}");
            }
        }

        private async Task SyncBCTC_HangKhong_CDKT(string code)
        {
            try
            {
                var time = GetCurrentTime();
                ...
                foreach (var item in lBatch)
                {
                    ...
                    for (int i = 0; i < count; i++)
                    {
                        ...
                        switch (i)
                        {
                            case 0: AssignData(VayNganHan?.Value1, VayDaiHan?.Value1, VonChuSH?.Value1); break;
                            case 1: AssignData(VayNganHan?.Value2, VayDaiHan?.Value2, VonChuSH?.Value2); break;
                            case 2: AssignData(VayNganHan?.Value3, VayDaiHan?.Value3, VonChuSH?.Value3); break;
                            case 3: AssignData(VayNganHan?.Value4, VayDaiHan?.Value4, VonChuSH?.Value4); break;
                            case 4: AssignData(VayNganHan?.Value5, VayDaiHan?.Value5, VonChuSH?.Value5); break;
                            case 5: AssignData(VayNganHan?.Value6, VayDaiHan?.Value6, VonChuSH?.Value6); break;
                            case 6: AssignData(VayNganHan?.Value7, VayDaiHan?.Value7, VonChuSH?.Value7); break;
                            case 7: AssignData(VayNganHan?.Value8, VayDaiHan?.Value8, VonChuSH?.Value8); break;
                            default: break;
                        };
            ...
            {
                _logger.LogError($"BllService.SyncBCTC_HangKhong_CDKT|EXCEPTION| {ex.Message}");
            }
        }
    }
}
