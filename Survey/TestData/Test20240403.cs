...
namespace Survey.TestData
{
    public class Test20240403
    {
        ...
        {
            var lPrint = _lResult.Select(x => new Info20240403VM
            {
                ...
                Has1DAboveMa20 = x.Has1DAboveMa20 == 0 ? "UNKNOWN" : (x.Has1DAboveMa20 == 1 ? "CO" : "KHONG")
            });
            ...
            //var tmp2 = 1;
            Console.WriteLine("Ngay Mua, Ngay Ban, So Nen, Take Profit(%), , Do Dai Nen(%), Do Dai Rau Nen(%), KC Tu Close Den Ma20(%), Do Rong BB(%), , Goc Tang, KC Day Den Ma20(%), Do Dai Nen Ban(%), Day Ngay, So Nen Tu Day, So Sanh Vol, Cung 1D");
        ...
        public static List<Info20240403> _lResult = new List<Info20240403>();
        ...
        {
            var lDataQuote = Data.GetDataAll(coin, EInterval.I4H).Select(x => x.To<Quote>()).ToList();
            var lDataQuote1D = Data.GetDataAll(coin, EInterval.I1D).Select(x => x.To<Quote>()).ToList();
            ...
            var lVol = Data.GetDataAll(coin, EInterval.I4H).Select(x => x.To<Quote>()).ToList();
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (_check2Sell)
                {
                    ...
                    //
                    var close = lDataQuote1D.FirstOrDefault(x => x.Date.Year == _ItemCheckCur.Date.Year && x.Date.Month == _ItemCheckCur.Date.Month && x.Date.Day == _ItemCheckCur.Date.Day)?.Close;
                    foreach (var item1D in lDataQuote1D)
                    {
                        if (item1D.Date.Year == _ItemCheckCur.Date.Year && item1D.Date.Month == _ItemCheckCur.Date.Month && item1D.Date.Day == _ItemCheckCur.Date.Day)
                        {
                            item1D.Close = _ItemCheckCur.Close;
                            break;
                        }
                    }

                    var lMa201D = lDataQuote1D.GetSma(20);
                    var entity1D = lDataQuote1D.First(x => x.Date.Year == _ItemCheckCur.Date.Year && x.Date.Month == _ItemCheckCur.Date.Month && x.Date.Day == _ItemCheckCur.Date.Day);
                    var index1D = lDataQuote1D.IndexOf(entity1D);
                    var entityMa201D = lMa201D.ElementAt(index1D);
                    foreach (var item1D in lDataQuote1D)
                    {
                        if (item1D.Date.Year == _ItemCheckCur.Date.Year && item1D.Date.Month == _ItemCheckCur.Date.Month && item1D.Date.Day == _ItemCheckCur.Date.Day)
                        {
                            item1D.Close = close ?? 0;
                            break;
                        }
                    }

                    var objPrint = new Info20240403
                    {
                        ...
                        Has1DAboveMa20 = entityMa201D.Sma is null ? 0 : ((close >= (decimal)entityMa201D.Sma) ? 1 : 2)
                    };
    ...
    public class Info20240403
    {
        ...
        public int Has1DAboveMa20 { get; set; }//Check cung 1d có ở trên Ma20 hay không 0: ko rõ; 1: trên; 2: dưới
    }

    public class Info20240403VM
    {
        ...
        public string Has1DAboveMa20 { get; set; }
    }
}
